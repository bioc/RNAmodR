...
is converted to a `TxDb` object and a `GRangesList` is retrieved using 
`exonsBy`.
...
Elements are returned as a `SequenceDataFrame` independent of the type of
`SequenceData` used.
...
argument `findMod` is not set to `FALSE`. For more informations on how to create
a `Modifier` object, have a look at the man page.
...
will make some things easier later on during visualization, which relies on the 
`Gviz` package)
...
The behavior of a `Modifier` class can be fine tuned, if the option is provided.
The `settings()` function is a getter/setter for arguments used in the analysis.
...
`Modifier` class itself uses the functionallity, it is switched off unless
...
via the `modifications()` function. The function returns a `GRanges` and
...
optional argument `perTranscript = TRUE`. In this example this will yield the
...
- adapter for using machine learning models for modification detection

We welcome contribution of any sort.
