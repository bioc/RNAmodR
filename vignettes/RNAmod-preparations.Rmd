...
fafile <- paste0(folder,"GRCh38_latest_genomic.fna")
tRNAscanfile <- paste0(folder,"hg38-tRNAs.ss.sort")
matchFastaToGff(gfffile,fafile)

gff <- rtracklayer::import.gff3(gfffile)
RDN18 <- S4Vectors::mcols(gff[grepl("RNA18",S4Vectors::mcols(gff)$ID),])$ID
RDN28 <- S4Vectors::mcols(gff[grepl("RNA28",S4Vectors::mcols(gff)$ID),])$ID
RDN5 <- S4Vectors::mcols(gff[grepl("RNA5",S4Vectors::mcols(gff)$ID),])$ID
