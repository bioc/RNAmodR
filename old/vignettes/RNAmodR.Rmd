...
mod_sce_rRNA_tRNA <- RNAmodR("test",
                             "sce_rRNA_tRNA.csv",
                             "sacCer3_R64-2-1_rRNA_append_rRNA_tRNA.gff3",
                             "sacCer3_R64-2-1_rRNA_append_rRNA_tRNA.fasta")

no <- unique(getExperimentData(mod)$ExperimentNo)
for(i in seq_along(no)){
  parseForModifications(mod_sce_rRNA_tRNA,no[i])
  se <- getSummarizedExperiment(mod,no[i])
  ...
  saveModPlot(mod,
              se,
              getExperimentData(mod)$Modifications,
              transcripts)
}
gc()
